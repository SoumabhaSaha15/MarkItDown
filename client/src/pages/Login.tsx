...
export const Login: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    setTitle("Login");
    auth.login(() => { navigate('/user/profile', { replace: true }) }, () => { navigate('/', { replace: true }) });
  }, []);
        ...
        <figure className="px-10 pt-10">
          <div className="avatar">
            <div className="w-24 rounded-full">
              <img src={(auth.userDetails !== null) ? auth.userDetails.profilePhoto : '/user-circle.svg'} alt={auth.userDetails?._id || 'uid'} />
            </div>
          </div>
        </figure>
